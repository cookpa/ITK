...
template <typename TInputImage, typename TOutputImage>
void
BSplineControlPointImageFilter<TInputImage, TOutputImage>::SetNumberOfLevels(ArrayType levels)
{
...
template <typename TInputImage, typename TOutputImage>
void
BSplineControlPointImageFilter<TInputImage, TOutputImage>::SetSplineOrder(unsigned int order)
{
...
template <typename TInputImage, typename TOutputImage>
void
BSplineControlPointImageFilter<TInputImage, TOutputImage>::SetSplineOrder(ArrayType order)
{
...
template <typename TInputImage, typename TOutputImage>
void
BSplineControlPointImageFilter<TInputImage, TOutputImage>::DynamicThreadedGenerateData(
  const OutputImageRegionType & region)
{
  ...
  for (unsigned int i = 0; i < ImageDimension; ++i)
  {
    ...
    {
      totalNumberOfSpans[i] = inputPtr->GetLargestPossibleRegion().GetSize()[i] - this->m_SplineOrder[i];
    }
  }
  ...
  for (ImageRegionIteratorWithIndex It(outputPtr, region); !It.IsAtEnd(); ++It)
  {
    ...
    for (unsigned int i = 0; i < ImageDimension; ++i)
    {
      ...
      if (itk::Math::Absolute(U[i] - static_cast<RealType>(totalNumberOfSpans[i])) <= epsilon[i])
      {
        U[i] = static_cast<RealType>(totalNumberOfSpans[i]) - epsilon[i];
      }
      if (U[i] < RealType{} && itk::Math::Absolute(U[i]) <= epsilon[i])
      {
        U[i] = RealType{};
      }
