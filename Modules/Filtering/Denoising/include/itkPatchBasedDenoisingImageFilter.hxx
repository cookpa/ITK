...
template <typename TInputImage, typename TOutputImage>
void
PatchBasedDenoisingImageFilter<TInputImage, TOutputImage>::EmptyCaches()
{
...
template <typename TInputImage, typename TOutputImage>
void
PatchBasedDenoisingImageFilter<TInputImage, TOutputImage>::SetThreadData(int threadId, const ThreadDataStruct & data)
{
...
template <typename TInputImage, typename TOutputImage>
void
PatchBasedDenoisingImageFilter<TInputImage, TOutputImage>::SetNoiseSigma(const RealType & sigma)
{
  m_NoiseSigma = sigma;
...
template <typename TInputImage, typename TOutputImage>
void
PatchBasedDenoisingImageFilter<TInputImage, TOutputImage>::SetKernelBandwidthSigma(const RealArrayType & kernelSigma)
{
...
template <typename TInputImage, typename TOutputImage>
void
PatchBasedDenoisingImageFilter<TInputImage, TOutputImage>::CopyInputToOutput()
{
...
template <typename TInputImage, typename TOutputImage>
void
PatchBasedDenoisingImageFilter<TInputImage, TOutputImage>::GenerateInputRequestedRegion()
{
...
template <typename TInputImage, typename TOutputImage>
void
PatchBasedDenoisingImageFilter<TInputImage, TOutputImage>::AllocateUpdateBuffer()
{
...
template <typename TInputImage, typename TOutputImage>
void
PatchBasedDenoisingImageFilter<TInputImage, TOutputImage>::EnforceConstraints()
{
...
template <typename TInputImage, typename TOutputImage>
void
PatchBasedDenoisingImageFilter<TInputImage, TOutputImage>::InitializePatchWeights()
{
...
template <typename TInputImage, typename TOutputImage>
void
PatchBasedDenoisingImageFilter<TInputImage, TOutputImage>::InitializePatchWeightsSmoothDisc()
{
  ...
  using ResampleFilterType = itk::ResampleImageFilter<WeightsImageType, WeightsImageType>;
...
template <typename TInputImageType>
void
PatchBasedDenoisingImageFilter<TInputImage, TOutputImage>::DispatchedArrayMinMax(const TInputImageType * img)
{
...
template <typename TInputImageType>
void
PatchBasedDenoisingImageFilter<TInputImage, TOutputImage>::DispatchedRiemannianMinMax(const TInputImageType * img)
{
                             ...
                             ThreadDataStruct             threadData)
{
  ...
  for (auto fIt = faceList.begin(); fIt != faceList.end(); ++fIt)
  {
    ...
    {
      this->ComputeLogMapAndWeightedSquaredGeodesicDifference(imgIt.Get(),
                                                              identityTensor,
                                                              identityWeight,
...
template <typename TInputImage, typename TOutputImage>
void
PatchBasedDenoisingImageFilter<TInputImage, TOutputImage>::ResolveRiemannianMinMax()
{
  ...
  for (unsigned int threadNum = 0; threadNum < numThreads; ++threadNum)
  {
    for (unsigned int ic = 0; ic < m_NumIndependentComponents; ++ic)
    {
      ...
      {
        itkDebugMacro("m_ThreadData[" << threadNum << "].minNorm[" << ic << "]: " << m_ThreadData[threadNum].minNorm[ic]
  ...
  RealArrayType &                           geodesicDist)
{
  ...
  Y[2] = (eigenVecs(2, 0) * factor0 + eigenVecs(2, 1) * factor1 + eigenVecs(2, 2) * factor2) /
         std::sqrt(eigenVals[0] * eigenVals[2]);
  ...
  Y[4] = (eigenVecs(2, 0) * factor0 + eigenVecs(2, 1) * factor1 + eigenVecs(2, 2) * factor2) /
         std::sqrt(eigenVals[1] * eigenVals[2]);
  ...
  -> RealType
{
  ...
  for (unsigned int pc = 0; pc < m_NumPixelComponents; ++pc)
  {
    this->SetComponent(result, pc, this->GetComponent(a, pc) + this->GetComponent(b, pc));
  }
  return result;
}
...
template <typename TInputImage, typename TOutputImage>
void
PatchBasedDenoisingImageFilter<TInputImage, TOutputImage>::InitializeKernelSigma()
{
  for (unsigned int ic = 0; ic < m_NumIndependentComponents; ++ic)
  {
...
template <typename TInputImage, typename TOutputImage>
void
PatchBasedDenoisingImageFilter<TInputImage, TOutputImage>::InitializeIteration()
{
  const PatchRadiusType radius = this->GetPatchRadiusInVoxels();
...
template <typename TInputImage, typename TOutputImage>
void
PatchBasedDenoisingImageFilter<TInputImage, TOutputImage>::ApplyUpdate()
{
  // Set up for multithreaded processing.
  ThreadFilterStruct str;
  str.Filter = this;
...
template <typename TInputImage, typename TOutputImage>
void
PatchBasedDenoisingImageFilter<TInputImage, TOutputImage>::ComputeKernelBandwidthUpdate()
{
  // Set up for multithreaded processing.
  ThreadFilterStruct str;
  ...
  this->GetMultiThreader()->SetNumberOfWorkUnits(this->GetNumberOfWorkUnits());
  this->GetMultiThreader()->SetSingleMethod(this->ComputeSigmaUpdateThreaderCallback, &str);
  ...
  {
    itkDebugMacro("Computing iteration " << i << " of kernel sigma update");
    ...
    itkDebugMacro("sigmaUpdate: " << sigmaUpdate << ", m_KernelBandwidthSigma: " << m_KernelBandwidthSigma
  ...
  ThreadDataStruct             threadData) -> ThreadDataStruct
{
  ...
  for (SampleIteratorType sampleIt = inList->Begin(); sampleIt != inList->End(); ++sampleIt, ++sampleNum)
  {
         ...
         ++selectedIt)
    {
      ...
      for (unsigned int ic = 0; ic < m_NumIndependentComponents; ++ic)
      {
        ...
        const RealValueType gaussianJointEntropy = exp(-itk::Math::sqr(distanceJointEntropy / sigmaKernel) / 2.0);

        probJointEntropy[ic] += gaussianJointEntropy;

        const RealValueType factorJoint = squaredNorm[ic] / pow(sigmaKernel, 3.0) - (lengthPatch * 1 / sigmaKernel);

        probJointEntropyFirstDerivative[ic] += gaussianJointEntropy * factorJoint;
        probJointEntropySecondDerivative[ic] +=
          gaussianJointEntropy * (itk::Math::sqr(factorJoint) + (lengthPatch * 1 / itk::Math::sqr(sigmaKernel)) -
        ...
        if (m_ComputeConditionalDerivatives)
        {
          ...
          const RealValueType gaussianPatchEntropy = exp(-itk::Math::sqr(distancePatchEntropy / sigmaKernel) / 2.0);
          probPatchEntropy[ic] += gaussianPatchEntropy;
          const RealValueType factorPatch =
            distancePatchEntropySquared / pow(sigmaKernel, 3.0) - ((lengthPatch - 1) / sigmaKernel);

          probPatchEntropyFirstDerivative[ic] += gaussianPatchEntropy * factorPatch;
          probPatchEntropySecondDerivative[ic] +=
            gaussianPatchEntropy * (itk::Math::sqr(factorPatch) + ((lengthPatch - 1) / itk::Math::sqr(sigmaKernel)) -
    ...
    for (unsigned int ic = 0; ic < m_NumIndependentComponents; ++ic)
    {
      if (!m_SigmaConverged[ic])
      {
        ...
        jointEntropyFirstDerivative[ic] -= probJointEntropyFirstDerivative[ic] / probJointEntropy[ic];
        jointEntropySecondDerivative[ic] -= probJointEntropySecondDerivative[ic] / probJointEntropy[ic] -
        ...
        if (m_ComputeConditionalDerivatives)
        {
          ...
          nbhdEntropyFirstDerivative[ic] -= probPatchEntropyFirstDerivative[ic] / probPatchEntropy[ic];
          nbhdEntropySecondDerivative[ic] -= probPatchEntropySecondDerivative[ic] / probPatchEntropy[ic] -
  ...
  for (unsigned int ic = 0; ic < m_NumIndependentComponents; ++ic)
  {
    if (!m_SigmaConverged[ic])
    {
      if (m_ComputeConditionalDerivatives)
      {
        threadData.entropyFirstDerivative[ic] = jointEntropyFirstDerivative[ic] - nbhdEntropyFirstDerivative[ic];
        threadData.entropySecondDerivative[ic] = jointEntropySecondDerivative[ic] - nbhdEntropySecondDerivative[ic];
      }
      else
      {
...
template <typename TInputImage, typename TOutputImage>
void
PatchBasedDenoisingImageFilter<TInputImage, TOutputImage>::ComputeImageUpdate()
{
  // Set up for multithreaded processing.
  ThreadFilterStruct str;
  ...
  ThreadDataStruct             threadData) -> ThreadDataStruct
{
  ...
  using SampleIteratorType = typename ListAdaptorType::ConstIterator;
  using GaussianOperatorType = GaussianOperator<RealValueType, OutputImageType::ImageDimension>;
        ...
        switch (this->GetNoiseModel())
        {
          ...
          case Superclass::NoiseModelEnum::RICIAN:
          {
            for (unsigned int pc = 0; pc < m_NumPixelComponents; ++pc)
            {
              ...
              const RealValueType gradientFidelity =
                (inVal * (gOper.ModifiedBesselI1(alpha) / gOper.ModifiedBesselI0(alpha)) - outVal) / sigmaSquared;
              const RealValueType stepSizeFidelity = sigmaSquared;
              // Update
              const RealValueType noiseVal = fidelityWeight * (stepSizeFidelity * gradientFidelity);
              // Ensure that the result is nonnegative
              this->SetComponent(
                result, pc, std::max(this->GetComponent(result, pc) + noiseVal, static_cast<RealValueType>(0.0)));
            }
            break;
          }
          case Superclass::NoiseModelEnum::POISSON:
          {
            for (unsigned int pc = 0; pc < m_NumPixelComponents; ++pc)
            {
              ...
              this->SetComponent(
                result, pc, std::max(this->GetComponent(result, pc) + noiseVal, static_cast<RealValueType>(0.00001)));
            }
            break;
          }
          default:
  ...
  ThreadDataStruct &                  threadData) -> RealType
{
       ...
       ++selectedIt)
  {
    ...
    for (unsigned int pc = 0; pc < m_NumPixelComponents; ++pc)
    {
      this->SetComponent(gradientJointEntropy,
                         pc,
                         GetComponent(gradientJointEntropy, pc) +
                           GetComponent(centerPatchDifference, pc) * gaussianJointEntropy);
    }
  } // end for each selected patch
